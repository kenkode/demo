...
class LoanrepaymentsController extends \BaseController {
	...
	{
		$loanrepayments = Loanrepayment::where('organization_id',Confide::user()->organization_id)
		->get();
		...
		switch ($recovered_status) {
			case 0:
					...
					$loanguarantors=DB::table('loanguarantors')
						...
						$loanrecover=Loanaccount::where('id','=',$loan_id)->first();
						...
						$loanaccountupdate->organization_id=Confide::User()->organization_id;
					...
					//Check whether there are no guarantors
						if(count($loanguarantors)<1){
							...
							foreach($loanguarantors as $loanguara){								
								...
								$guarant->organization_id=Confide::User()->organization_id;	
								$guarant->save();								
							}
							Loanrepayment::repayLoan($data);
	...
	public function store()
	{
		...
		$loanamount=Loanaccount::where('id','=',$loanaccount)
		->where('organization_id',Confide::user()->organization_id)
		->pluck('amount_applied');
		//dd($loanamount);
		$repayamount= Input::get('amount');
		$guarantors=DB::table('loanguarantors')
