  ...
  <div class="col-lg-12">
    ...
    <table id="example" data-show-refresh="true" style="font-size:10px;width:1000px" class="table table-condensed table-bordered table-responsive table-hover nowrap">
      ...
      <tbody class="displayrecord">
        ...
        <?php Payroll::remittances($employee->member_id);?>
        <tr>
          ...
          <td align="right">{{ asMoney((double)Member::getMember($employee->member_id)) }}</td>
          <?php
          ...
          <td align="right">{{ asMoney((double)Payroll::loans($employee->member_id,$loanaccount,$period)) }}</td>
          <?php
          ...
          <?php
           ...
           $totaldeduction = $totaldeduction + (double)Payroll::total_deductions($employee->id,$period)+$totalcbs;
          ?>
          <?php
           $totalnet = $totalnet + (double)Payroll::net($employee->id,$period)-$totalcbs;
          ?>
          <td align="right"><strong>{{ asMoney($totalcbs + (double)Payroll::total_deductions($employee->id,$period)) }}</strong></td>
          <td align="right"><strong>{{ asMoney((double)Payroll::net($employee->id,$period)-$totalcbs) }}</strong></td>
        ...
        <tr style="background:#EEE;">
          ...
          <td align='right'><strong>{{asMoney($totalsalary)}}</strong></td>
           @foreach($earnings as $earning)
          <?php
           $totalearning = $totalearning + (double)Payroll::totalearnings($earning->id,$period);
          ?>
          <td align='right'><strong>{{asMoney($totalearning)}}</strong></td>
          @endforeach
          <td align='right'><strong>{{asMoney($totalhourly)}}</strong></td>
          <td align='right'><strong>{{asMoney($totaldaily)}}</strong></td>
          @foreach($allowances as $allowance)
          <?php
           $totalallowance = $totalallowance + (double)Payroll::totalallowances($allowance->id,$period);
          ?>
          <td align='right'><strong>{{asMoney($totalallowance)}}</strong></td>
          @endforeach
          
          <td align='right'><strong>{{asMoney($totalgross)}}</strong></td>
           @foreach($nontaxables as $nontaxable)
          <?php
           $totalnontaxable = $totalnontaxable + (double)Payroll::totalnontaxable($nontaxable->id,$period);
          ?>
          <td align='right'><strong>{{asMoney($totalnontaxable)}}</strong></td>
          @endforeach
          <td align='right'><strong>{{asMoney($totaltax)}}</strong></td>
          <td align='right'><strong>{{asMoney($totaltaxrelief)}}</strong></td>
          @foreach($reliefs as $relief)
          <?php
           $totalrelief = $totalrelief + (double)Payroll::totalreliefs($relief->id,$period);
          ?>
          <td align='right'><strong>{{asMoney($totalrelief)}}</strong></td>
          @endforeach
          ...
          @foreach($deductions as $deduction)
          <?php
           $otherdeduction = $otherdeduction + (double)Payroll::totaldeductions($deduction->id,$period);
          ?>
          <td align='right'><strong>{{asMoney($otherdeduction)}}</strong></td>
          @endforeach
          <td align='right'><strong>{{asMoney($totalrem)}}</strong></td>
          @foreach($loanproducts as $loanproduct)
          <?php
           $totalloans = $totalloans + (double)Payroll::totalloans($loanproduct->id);
          ?>
          <td align='right'><strong>{{asMoney($totalloans)}}</strong></td>
          @endforeach
