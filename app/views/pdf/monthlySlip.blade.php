    ...
    <table class="table table-bordered" border='1' cellspacing='0' cellpadding='0' style='width:350px'>
      ...
      <tr><td>Kra Pin: </td>
       ...
       <tr><td>Paye: </td><td align='right'>{{ Payroll::processedpaye($emp->personal_file_number,$period) }}</td></tr>
        ...
        <tr><td>Monthly Remittance: </td><td align='right'>{{ Payroll::processedremittances($emp->personal_file_number,$period) }}</td></tr>

        @if(Payroll::processedloanproductnames($emp->id,$period)  != null)
        <tr><td>{{ Payroll::processedloanproductnames($emp->id,$period) }}: </td><td align='right'>{{ Payroll::processedLoans($emp->id,$period) }}</td></tr>
        @else
        @endif
  ...
  <div  class="content" style='margin-top:-50px;'>
     <table class="table table-bordered" border='1' cellspacing='0' cellpadding='0' style='width:350px'>
      ...
      <tr><td>Kra Pin: </td>
       ...
       <tr><td>Monthly Remittance: </td><td align='right'>{{ Payroll::asMoney($transact->monthly_remittance) }}</td></tr>

       @foreach($loans as $loan)
        @if($loan->loanproduct_name != null || $loan->loanproduct_name != '')
        <tr><td>{{ $loan->loanproduct_name }}: </td><td align='right'>{{ Payroll::asMoney($loan->amount) }}</td></tr>
        @else
        @endif
       @endforeach
