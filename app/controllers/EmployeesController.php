...
class EmployeesController extends \BaseController {
    ...
    $emps = Employee::where('organization_id',Confide::user()->organization_id)->get();
    ...
    if(Employee::where('employee.organization_id',Confide::user()->organization_id)->orderBy('id', 'DESC')->count() == 0){
      ...
      $pfn = preg_replace('/\D/', '', $pfn);
		...
		return View::make('employees.create', compact('currency','groups','emps','citizenships','pfn','branches','departments','etypes','jgroups','banks','bbranches','educations'));
	}
  }
  ...
  $memsign = '';
       ...
       if ( Input::hasFile('image')) {
            ...
            $file = $file->move('public/uploads/photos/', $name);
            $input['file'] = '/public/uploads/photos/'.$name;
            $employee->photo = $name;
            $memphoto = $name;
        }else{
        	$employee->photo = 'default_photo.png';
          $memphoto = 'default_photo.png';
        }
    ...
    if(Input::get('is_member') == '1'){
    $member = new Member;

    $member->photo = $memphoto;
    $member->signature = $memsign;
    ...
    $member->id_number = Input::get('identity_number');
    $member->first_name = Input::get('fname');
    $member->last_name = Input::get('lname');
    $member->middle_name = Input::get('mname');
    $member->membership_no = Input::get('personal_file_number');
    $member->phone = Input::get('telephone_mobile');
    $member->email = Input::get('email_office');
    $member->address = Input::get('address');
    $member->bank_id = Input::get('bank_id');
    $member->bank_branch_id = Input::get('bbranch_id');
    $member->bank_account_number = Input::get('bank_account_number');
    ...
    Shareaccount::createAccount($member->id);

    $empupdate = Employee::find($insertedId);

    $empupdate->member_id = $member->id;
    ...
    for($i=0;$i<count(Input::get('kin_first_name'));$i++){
        ...
        $memkin = new Kin;
        $memkin->member_id=$employee->member_id;
        $memkin->first_name = Input::get('kin_first_name')[$i];
        $memkin->last_name = Input::get('kin_last_name')[$i];
        $memkin->middle_name = Input::get('kin_middle_name')[$i];
        if(Input::get('kin_middle_name')[$i] != "" || Input::get('kin_middle_name')[$i] != null){
          $memkin->name = Input::get('kin_first_name')[$i]." ".Input::get('kin_middle_name')[$i]." ".Input::get('kin_last_name')[$i];
        }else{
          $memkin->name = Input::get('kin_first_name')[$i]." ".Input::get('kin_last_name')[$i];
        }
        $memkin->rship = Input::get('relationship')[$i];
        $memkin->contact = Input::get('contact')[$i];
        $memkin->id_number = Input::get('id_number')[$i];
        $memkin->organization_id=Confide::User()->organization_id;
        $memkin->save();
  ...
  public function serializeDoc(){
    ...
    $subordinates = Employee::where('organization_id',Confide::user()->organization_id)->get();
    $groups = Group::where('organization_id',Confide::user()->organization_id)->get();
    $currency = Currency::whereNull('organization_id')->orWhere('organization_id',Confide::user()->organization_id)->first();
		return View::make('employees.edit', compact('currency','groups','member','supervisor','count','subordinates','countk','countd','docs','kins','citizenships','contract','branches','educations','departments','etypes','jgroups','banks','bbranches','employee'));
	}
    ...
    if(Input::get('is_member') == '1' && ($employee->member_id == null || $employee->member_id == 0)){
    $insertedId = $employee->id;
    ...
    $empupdate->update();
    }else if(Input::get('is_member') == '1' && $employee->member_id > 0){
     $member = Member::find($employee->member_id);
    ...
    Kin::where('member_id', $employee->member_id)
    ...
    $c = Supervisor::where('employee_id', $id)->count();
		...
		return View::make('employees.view', compact('employee','member','c','appraisals','kins','documents','occurences','properties','count','benefits'));
    ...
    $c = Supervisor::where('employee_id', $id)->count();
		...
		return View::make('employees.viewdeactive', compact('employee','member','c','appraisals','kins','documents','occurences','properties','count','benefits'));
