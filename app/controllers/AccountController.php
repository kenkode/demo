...
public function store()
	...
	{
		$banking = Accounts::where('organization_id',Confide::User()->organization_id)->get();
		$account = Banking::where('organization_id',Confide::User()->organization_id)->get();
	...
	public function recordbanking()
	{
		...
		DB::table('account')->leftjoin('banking','account.name','=','banking.account_from')
		->where('account.name', '=', $banking->account_from)
		->where('organization_id',Confide::User()->organization_id)
		->decrement('account.balance', $banking->amount);
		DB::table('account')->leftjoin('banking','account.name','=','banking.account_to')
		->where('account.name', '=', $banking->account_to)
		->where('organization_id',Confide::User()->organization_id)
		->increment('account.balance', $banking->amount);
