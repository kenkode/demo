...
class ProjectsController extends \BaseController{
	...
	*/
	public function index(){
		$projects=Project::where('organization_id',Confide::User()->organization_id)->get();
	...
	*/
	public function store(){
		...
		if($invest==null){
			...
			$project->organization_id=Confide::User()->organization_id;
			$project->save();
	...
	*/
	public function show(){
		...
		if($usertype=='admin'){
			...
			$projectile=Project::where('units','>',0)->where('organization_id',Confide::User()->organization_id)->get();
			$approvedones=Projectorder::where('is_approved','=',1)->where('organization_id',Confide::User()->organization_id)->get();
	...
	*/
	public function createOrder($id){
		$project=Project::where('id','=',$id)->where('organization_id',Confide::User()->organization_id)->get()->first();
	...
	*/
	public function docreateOrder(){
		...
		$units=Project::where('id','=',$prid)->where('organization_id',Confide::User()->organization_id)->pluck('units');
		if($prunits>$units){
			...
			$projectile=Project::where('organization_id',Confide::User()->organization_id)->get();
	...
	*/
	public function approve($id){
		$project=Projectorder::where('id','=',$id)->where('organization_id',Confide::User()->organization_id)->get()->first();
	...
	*/
	public function doapprove(){
		...
		$members=Member::where('id','=',$member)->where('organization_id',Confide::User()->organization_id)->get()->first();
		//Get payment mode
		$paymode=$project->payment_mode;
		$id=$project->project_id;
		$units=$project->units;
		//get the project unit price from table projects
		$unit_price=Project::where('id','=',$id)->where('organization_id',Confide::User()->organization_id)->pluck('unit_price');
		...
		*/
		switch($paymode){
			case $paymode=='Loan Transfer':
				...
				$loans=Loanaccount::where('member_id','=',$member)->where('organization_id',Confide::User()->organization_id)->count();
				if($loans<6){
					//Approve the project
					...
					$reduce=Project::where('id','=',$project->project_id)->where('organization_id',Confide::User()->organization_id)->get()->first();
					...
					$loanproduct=Loanproduct::orderBy('id','DESC')->where('organization_id',Confide::User()->organization_id)->get()->first();
					...
					$apply->organization_id=Confide::user()->organization_id;				
					$apply->save();
					$date=date('Y-m-d');
	...
	*/
	public function doreject(){
		$data=Input::all();
		$project=Projectorder::where('id','=',array_get($data,'project'))->where('organization_id',Confide::User()->organization_id)->get()->first();	
		//Obtain member details
		$member=$project->member_id;		
		$members=Member::where('id','=',$member)->where('organization_id',Confide::User()->organization_id)->get()->first();		
	...
	*/
	public function doupdateOrder(){
		$data=Input::all();
		$prid=array_get($data,'project');
		$pr=Projectorder::where('id','=',$prid)->where('organization_id',Confide::User()->organization_id)->get()->first();
		$prunits=array_get($data,'units');
		$units=Project::where('id','=',$pr->project_id)->where('organization_id',Confide::User()->organization_id)->pluck('units');
		if($prunits>$units || $prunits<1){
			return Redirect::back()->withNone('Units applied cannot be more than available units');
		}else{
			$project=Projectorder::where('id','=',$prid)
	...
	*/
	public function doupdate(){
		...
		if($invest==null){
			...
			$project=Project::where('id','=',array_get($data,'id'))
			->where('organization_id',Confide::User()->organization_id)->get()->first();
			$project->name=array_get($data,'name');
			$project->investment_id=array_get($data,'investment');
			$project->units=array_get($data,'units');
