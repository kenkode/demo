...
class PayrollController extends \BaseController {
    ...
    public function createaccount()
  {
    ...
    public function create()
    {
        ...
        return View::make('payroll.preview', compact('employees','loanproducts','savingproducts','period','account','nontaxables','earnings','overtimes','allowances','reliefs','deductions'));
    }
    ...
    $data8    = DB::table('transact_loans')->where('organization_id',Confide::user()->organization_id)->where('financial_month_year', '=', $period)->delete();
...
//return View::make('payroll.payroll_calculator', compact('net','paye1','nssf1','nhif1','gross','currency'));
    ...
    public function display(){
      ...
      $employees = DB::table('employee')
        ...
        foreach($employees as $employee){
         ...
         if($employee->income_tax_applicable == 1 && (double)Payroll::gross($employee->id,$fperiod)>=11135.67 && $employee->income_tax_relief_applicable == 1){
        ...
        <tr>
          ...
          $display .="<td align='right'>".number_format((double)Member::getMember($employee->member_id),2)."</td>";

          $totalcbs = $totalcbs + (double)Member::getMember($employee->member_id);
          $totalrem = $totalrem + (double)Member::getMember($employee->member_id);

           if(count(Loanaccount::MemberLoan($employee->member_id)) == 0){
           $display .="<td align='right'>0.00</td>";
           }else{
           foreach(Loanaccount::MemberLoan($employee->member_id) as $loanaccount){
           $otherdeduction = $otherdeduction + (double)Payroll::loans($employee->member_id,$loanaccount,$period);
           $totalcbs = $totalcbs + (double)Payroll::loans($employee->member_id,$loanaccount,$period);
           $display .="<td align='right'><strong>".number_format((double)Payroll::loans($employee->member_id,$loanaccount,$period),2)."</strong></td>";
           }
           }

           $total_deductions = $total_deductions + $totalcbs;
           $net = $net - $totalcbs;
          ...
          <td align='right'><strong>".number_format($totalsalary,2)."</strong></td>";
           foreach($earnings as $earning){
           $totalearning = $totalearning + (double)Payroll::totalearnings($earning->id,$fperiod);
           ...
           foreach($allowances as $allowance){
           $totalallowance = $totalallowance + (double)Payroll::totalallowances($allowance->id,$fperiod);
           $display .="<td align='right'><strong>".number_format($totalallowance,2)."</strong></td>";
           }
          
           $display .="<td align='right'><strong>".number_format($totalgross,2)."</strong></td>";
           foreach($nontaxables as $nontaxable){
           $totalnontaxable = $totalnontaxable + (double)Payroll::totalnontaxable($nontaxable->id,$fperiod);
           $display .="<td align='right'><strong>".number_format($totalnontaxable,2)."</strong></td>";
           }
          $display .="<td align='right'><strong>$totaltax</strong></td>
          <td align='right'><strong>".number_format($totaltaxrelief,2)."</strong></td>";
          foreach($reliefs as $relief){
           $totalrelief = $totalrelief + (double)Payroll::totalreliefs($relief->id,$fperiod);
           $display .="<td align='right'><strong>".number_format($totalrelief,2)."</strong></td>";
           }
          ...
          foreach($deductions as $deduction){
           $otherdeduction = $otherdeduction + (double)Payroll::totaldeductions($deduction->id,$fperiod);
           $display .="<td align='right'><strong>".number_format($otherdeduction,2)."</strong></td>";
           }
           $display .="<td align='right'>".number_format($totalrem,2)."</td>";
           if(count(Loanaccount::MemberLoan($employee->member_id)) == 0){
           $display .="<td align='right'>0.00</td>";
           }else{
           foreach($loanproducts as $loanproduct){
           $totalloans = $totalloans + (double)Payroll::totalloans($loanproduct->id);
           $display .="<td align='right'><strong>".number_format($totalloans,2)."</strong></td>";
           }
           }
          $display .="<td align='right'><strong>".number_format($totaldeduction+$totalcbs,2)."</strong></td>
          <td align='right'><strong>".number_format($totalnet-$totalcbs,2)."</strong></td>
        </tr>
        ";
    ...
    public function store()
    {
        ...
        $loans = Loanaccount::where('is_approved',1)
                              ->where('loan_status','open')
                              ->where('organization_id',Confide::User()->organization_id)
                              ->get();

        $countl = Loanaccount::where('is_approved',1)
                              ->where('loan_status','open')
                              ->where('organization_id',Confide::User()->organization_id)
                              ->count();
        ...
        if(Loanaccount::Employee($loan->member_id)->member_id != null || Loanaccount::Employee($loan->member_id)->member_id != 0){

        DB::table('transact_loans')->insert(
        ['employee_id' => Loanaccount::Employee($loan->member_id)->id, 
        'organization_id' => Confide::user()->organization_id,
        'member_id' => $loan->member_id, 
        'loanproduct_name' => $loan->loanproduct->name,
        'loanproduct_id' => $loan->loanproduct_id,
        'amount' => Payroll::loans($loan->member_id,$loan,Input::get('period')),
        'financial_month_year'=>Input::get('period')
        ]
        );
        }

        $totalcbs = $totalcbs + Payroll::loans($loan->member_id,$loan,Input::get('period'));
        ...
        $payroll->other_deductions = Payroll::deductionall($employee->id,Input::get('period'))+Payroll::remittances($employee->member_id)+Payroll::totalmemberloans($employee->member_id);
        $payroll->monthly_remittance = Payroll::remittances($employee->member_id);
        $payroll->total_deductions = Payroll::total_deductions($employee->id,Input::get('period'))+Payroll::remittances($employee->member_id)+Payroll::totalmemberloans($employee->member_id);
        $payroll->net = Payroll::net($employee->id,Input::get('period'))-Payroll::remittances($employee->member_id)-Payroll::totalmemberloans($employee->member_id);
