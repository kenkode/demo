...
class ReportsController extends \BaseController {
    ...
    public function period_payslip()
  {
    ...
    return $pdf->stream('Monthly_Payslip_'.$period.'.pdf');
      ...
      if(Input::get('format') == "excel"){
        ...
        $loans = DB::table('transact_loans')
            ->join('employee', 'transact_loans.employee_id', '=', 'employee.id')
            ->where('financial_month_year' ,'=', Input::get('period'))
            ->where('employee.id' ,'=', Input::get('employeeid'))
            ->where('employee.organization_id',Confide::user()->organization_id)
            ->groupBy('loanproduct_name')
            ->get(); 
  ...
  Excel::create('Payslips', function($excel) use($data,$nontaxables,$name,$period,$employee,$allws,$earnings,$overtimes,$rels,$deds,$organization,$loans,$currency) {
   ...
   $objPHPExcel->setActiveSheetIndex(0); 
    ...
    $excel->sheet('Payslip', function($sheet) use($data,$nontaxables,$name,$period,$employee,$allws,$earnings,$overtimes,$rels,$deds,$organization,$currency,$loans,$objPHPExcel){
            ...
            $sheet->row($row, array(
             ...
             $sheet->row($r+5, array(
              'Monthly Remittance:',$data->monthly_remittance
              ));

              $sheet->cell('B'.($r+5), function($cell) {
             ...
             $sheet->row($c, array(
             $loans[$i]->loanproduct_name,$loans[$i]->amount
             ));
  ...
  Excel::create($save.'_'.$month.' Payslip', function($excel) use($data,$nontaxables,$name,$period,$employee,$allws,$earnings,$overtimes,$rels,$deds,$organization,$loans,$currency) {
   ...
   $objPHPExcel->setActiveSheetIndex(0); 
    ...
    $pdf = PDF::loadView('pdf.monthlySlip', compact('nontaxables','empall','select','name','employee','transact','allws','deds','earnings','overtimes','rels','loans','period','currency', 'organization','id'))->setPaper('a5')->setOrientation('potrait');
...
'
   ...
   public function period_summary()
  {
    ...
    public function paySummary(){
        ...
        $total_rem = DB::table('transact')
        ->where('organization_id',Confide::user()->organization_id)
        ->where('financial_month_year' ,'=', Input::get('period'))
        ->sum('monthly_remittance');
        ...
        $data_loan = DB::table('transact_loans')
            ->join('employee', 'transact_loans.employee_id', '=', 'employee.id')
            ->where('financial_month_year' ,'=', Input::get('period'))
            ->where('employee.organization_id',Confide::user()->organization_id)
            ->select(DB::raw('DISTINCT(loanproduct_name) as loanproduct_name'))
            ->get(); 
  ...
  Excel::create('Payroll Summary '.$month, function($excel) use($data,$data_nontax,$data_earnings,$data_allowance,$data_loan,$data_overtime,$data_overtime_hourly,$data_overtime_daily,$data_relief,$data_deduction,$total_pay,$total_earning,$total_rem,$total_gross,$total_paye,$total_nssf,$total_nhif,$total_others,$total_deds,$total_net,$organization,$currency) {
   ...
   $objPHPExcel->setActiveSheetIndex(0); 
    ...
    $excel->sheet('Payroll Summary', function($sheet) use($data,$data_nontax,$data_earnings,$data_allowance,$data_loan,$data_overtime,$data_overtime_hourly,$data_overtime_daily,$data_relief,$data_deduction,$total_pay,$total_earning,$total_gross,$total_paye,$total_rem,$total_nssf,$total_nhif,$total_others,$total_deds,$total_net,$organization,$currency,$objPHPExcel){
              ...
              ->select(DB::raw('DISTINCT(deduction_name) as deduction_name'))
              ->get();
              $loanproducts = DB::table('transact_loans')
              ->where('financial_month_year' ,'=', Input::get('period'))
              ->select(DB::raw('DISTINCT(loanproduct_name) as loanproduct_name'))
              ->get();
              ...
              $columnLetter10 = PHPExcel_Cell::stringFromColumnIndex($colIndex+count($data_earnings)+count($data_allowance)+count($data_nontax)+count($data_relief)+count($deductions)+7); 

              $sheet->SetCellValue($columnLetter10.$row,"MONTHLY REMITTANCE");

              $colIndexded = PHPExcel_Cell::columnIndexFromString(PHPExcel_Cell::stringFromColumnIndex($colIndex+count($data_earnings)+count($data_allowance)+count($data_nontax)+count($data_relief)+count($deductions)));
              
              $columnLetter11 = PHPExcel_Cell::stringFromColumnIndex($colIndexded+7); 
             
              for ($column = $columnLetter11,$m=0; $column != PHPExcel_Cell::stringFromColumnIndex($colIndex+count($data_earnings)+count($data_allowance)+count($data_nontax)+count($data_relief)+count($deductions)+count($loanproducts)),$m<count($loanproducts); $column++,$m++) {
               
               $sheet->setCellValue($column.$row, strtoupper($loanproducts[$m]->loanproduct_name));
              } 


              $columnLetter4 = PHPExcel_Cell::stringFromColumnIndex($colIndex+count($data_earnings)+count($data_allowance)+count($data_nontax)+count($data_relief)+count($deductions)+count($loanproducts)+8); 

              $sheet->SetCellValue($columnLetter4.$row,"TOTAL DEDUCTIONS");

              $columnLetter5 = PHPExcel_Cell::stringFromColumnIndex($colIndex+count($data_earnings)+count($data_allowance)+count($data_nontax)+count($data_relief)+count($deductions)+count($loanproducts)+9); 
            ...
            $rg = 8;
             ...
             for($h=0;$h<count($data);$h++){
              ...
              for($q = 0; $q<count($data); $q++){
               ...
               $totalrem  = $totalrem  + $data[$q]->monthly_remittance;
               $rp++;
              ...
              for($re = 0; $re<count($data); $re++){
               
               $sheet->setCellValue(PHPExcel_Cell::stringFromColumnIndex($colIndex+count($data_earnings)+count($data_allowance)+count($data_nontax)+count($data_relief)+count($data_deduction)+7).$rr, $data[$re]->monthly_remittance);
              ...
              $colIndexded = PHPExcel_Cell::columnIndexFromString(PHPExcel_Cell::stringFromColumnIndex($colIndex+count($data_earnings)+count($data_allowance)+count($data_nontax)+count($data_relief)+count($data_deduction)));

              $columnLetter12 = PHPExcel_Cell::stringFromColumnIndex($colIndexded+7); 

              $rloan = 8;

              for($ln = 0; $ln<count($data); $ln++){
             
              for ($column = $columnLetter12,$l=0; $column != PHPExcel_Cell::stringFromColumnIndex($colIndex+count($data_earnings)+count($data_allowance)+count($data_nontax)+count($data_relief)+count($data_deduction)+count($data_loan)),$l<count($data_loan); $column++,$l++) {
               
               $sheet->setCellValue($column.$rloan, Payroll::transactloans($data[$ln]->personal_file_number,$data_loan[$l]->loanproduct_name,Input::get("period")));
              } 
              ...
              for($u = 0; $u<count($data); $u++){
               ...
               $sheet->setCellValue(PHPExcel_Cell::stringFromColumnIndex($colIndex+count($data_earnings)+count($data_allowance)+count($data_nontax)+count($data_relief)+count($deductions)+count($loanproducts)+8).$rn, round($data[$u]->total_deductions,2));
               $sheet->setCellValue(PHPExcel_Cell::stringFromColumnIndex($colIndex+count($data_earnings)+count($data_allowance)+count($data_nontax)+count($data_relief)+count($deductions)+count($loanproducts)+9).$rn, round($data[$u]->net,2));
               ...
               $rn++;
                ...
                $sheet->setCellValue(PHPExcel_Cell::stringFromColumnIndex($colIndex+count($data_earnings)+count($data_allowance)+count($data_nontax)+count($data_relief)+count($deductions)+7).$rn, $totalrem);

                for ($column = $columnLetter12,$l=0; $column != PHPExcel_Cell::stringFromColumnIndex($colIndex+count($data_earnings)+count($data_allowance)+count($data_nontax)+count($data_relief)+count($data_deduction)+count($data_loan)),$l<count($data_loan); $column++,$l++) {
               
                 $sheet->setCellValue($column.$rloan, Payroll::totaltransactloans($data_loan[$s]->loanproduct_name,'All','All',Input::get("period")));
                } 

               $sheet->setCellValue(PHPExcel_Cell::stringFromColumnIndex($colIndex+count($data_earnings)+count($data_allowance)+count($data_nontax)+count($data_relief)+count($deductions)+count($data_loan)+8).$rn, round($totaldeduction,2));
               $sheet->setCellValue(PHPExcel_Cell::stringFromColumnIndex($colIndex+count($data_earnings)+count($data_allowance)+count($data_nontax)+count($data_relief)+count($deductions)+count($data_loan)+9).$rn, round($totalnet,2));
  ...
  Excel::create('Payroll Summary '.$month, function($excel) use($sels,$data,$data_nontax,$data_earnings,$data_allowance,$data_loan,$total_rem,$data_overtime,$data_overtime_hourly,$data_overtime_daily,$data_relief,$data_deduction,$total_pay,$total_earning,$total_gross,$total_paye,$total_nssf,$total_nhif,$total_others,$total_deds,$total_net,$organization,$currency) {
   ...
   $objPHPExcel->setActiveSheetIndex(0); 
    ...
    $excel->sheet('Payroll Summary', function($sheet) use($sels,$data,$data_nontax,$data_earnings,$data_allowance,$data_loan,$total_rem,$data_overtime,$data_overtime_hourly,$data_overtime_daily,$data_relief,$data_deduction,$total_pay,$total_earning,$total_gross,$total_paye,$total_nssf,$total_nhif,$total_others,$total_deds,$total_net,$organization,$currency,$objPHPExcel){
              ...
              for($u = 0; $u<count($data); $u++){
               ...
               $sheet->setCellValue(PHPExcel_Cell::stringFromColumnIndex($colIndex+count($data_earnings)+count($data_allowance)+count($data_nontax)+count($data_relief)+count($deductions)+7).$rn, $totalrem);
                ...
                for ($column = $columnLetter12,$l=0; $column != PHPExcel_Cell::stringFromColumnIndex($colIndex+count($data_earnings)+count($data_allowance)+count($data_nontax)+count($data_relief)+count($data_deduction)+count($data_loan)),$l<count($data_loan); $column++,$l++) {
                 ...
                 $sheet->setCellValue($column.$rloan, Payroll::totaltransactloans($data_loan[$s]->loanproduct_name,Input::get('branch'),'All',Input::get("period")));
                } 
              ...
              for($u = 0; $u<count($data); $u++){
               ...
               $sheet->setCellValue(PHPExcel_Cell::stringFromColumnIndex($colIndex+count($data_earnings)+count($data_allowance)+count($data_nontax)+count($data_relief)+count($deductions)+7).$rn, $totalrem);
                ...
                for ($column = $columnLetter12,$l=0; $column != PHPExcel_Cell::stringFromColumnIndex($colIndex+count($data_earnings)+count($data_allowance)+count($data_nontax)+count($data_relief)+count($data_deduction)+count($data_loan)),$l<count($data_loan); $column++,$l++) {
                 ...
                 $sheet->setCellValue($column.$rloan, Payroll::totaltransactloans($data_loan[$s]->loanproduct_name,'All',Input::get('department'),Input::get("period")));
                } 
  ...
  Excel::create('Payroll Summary '.$month, function($excel) use($selBr,$selDt,$data,$data_nontax,$data_earnings,$data_loan,$total_rem,$data_allowance,$data_overtime,$data_overtime_hourly,$data_overtime_daily,$data_relief,$data_deduction,$total_pay,$total_earning,$total_gross,$total_paye,$total_nssf,$total_nhif,$total_others,$total_deds,$total_net,$organization,$currency) {
   ...
   $objPHPExcel->setActiveSheetIndex(0); 
    ...
    $excel->sheet('Payroll Summary', function($sheet) use($selBr,$selDt,$data,$data_nontax,$data_earnings,$data_allowance,$data_loan,$total_rem,$data_overtime,$data_overtime_hourly,$data_overtime_daily,$data_relief,$data_deduction,$total_pay,$total_earning,$total_gross,$total_paye,$total_nssf,$total_nhif,$total_others,$total_deds,$total_net,$organization,$currency,$objPHPExcel){
              ...
              for($u = 0; $u<count($data); $u++){
               ...
               $sheet->setCellValue(PHPExcel_Cell::stringFromColumnIndex($colIndex+count($data_earnings)+count($data_allowance)+count($data_nontax)+count($data_relief)+count($deductions)+7).$rn, $totalrem);
                ...
                for ($column = $columnLetter12,$l=0; $column != PHPExcel_Cell::stringFromColumnIndex($colIndex+count($data_earnings)+count($data_allowance)+count($data_nontax)+count($data_relief)+count($data_deduction)+count($data_loan)),$l<count($data_loan); $column++,$l++) {
                 ...
                 $sheet->setCellValue($column.$rloan, Payroll::totaltransactloans($data_loan[$s]->loanproduct_name,Input::get('branch'),Input::get('department'),Input::get("period")));
                } 
  ...
  Excel::create('Leave Balance Report ', function($excel) use($data,$leavetype,$organization) {
   ...
   $objPHPExcel->setActiveSheetIndex(0); 
    ...
    $excel->sheet('Leave Balance Report', function($sheet) use($data,$leavetype,$organization,$objPHPExcel){
              ...
              $sheet->row(5, array(
              'PAYROLL NUMBER', 'EMPLOYEE NAME', 'LEAVE DAYS', 'PAY RATE', 'TOTAL VALUE'
              ));
            ...
            $totalvalue = 0;
             ...
             $totaldays  = $totaldays  + Leaveapplication::getBalanceDays($data[$i], $leavetype);
             $totalpay   = $totalpay   + ($data[$i]->basic_pay/ 30);
             $totalvalue = $totalvalue + (Leaveapplication::getBalanceDays($data[$i], $leavetype) * ( $data[$i]->basic_pay/ 30));
      

             $sheet->row($row, array(
             $data[$i]->personal_file_number,$name,Leaveapplication::getBalanceDays($data[$i], $leavetype),round(($data[$i]->basic_pay/30),2),round(Leaveapplication::getBalanceDays($data[$i], $leavetype) * ($data[$i]->basic_pay/30),2)
             ));
             $row++;
             }             
             $sheet->row($row, array(
             '','Total',$totaldays,round($totalpay,2),round($totalvalue,2)
             ));
    });
  ...
  Excel::create('Leave Report for '.$employee->personal_file_number.' : '.$name, function($excel) use($data,$name,$employee,$organization) {
   ...
   $objPHPExcel->setActiveSheetIndex(0); 
    ...
    $excel->sheet('Leave Report', function($sheet) use($data,$name,$employee,$organization,$objPHPExcel){
              ...
              $sheet->row(5, array(
              'LEAVE TYPE', 'BEGINNING BALANCE', 'LEAVE TAKEN','LEAVE BALANCE','PAY RATE','TOTAL VALUE'
              ));
            ...
            $row = 6;
             ...
             $sheet->row($row, array(
             $application->leavetype->name,Leaveapplication::getBalanceDays($employee, $application->leavetype),Leaveapplication::getDaysTaken($employee, $application->leavetype),(Leaveapplication::getBalanceDays($employee, $application->leavetype))-(Leaveapplication::getDaysTaken($employee, $application->leavetype)),round($employee->basic_pay/30,2),round($bal * $employee->basic_pay/30,2)
             ));
             break;
             }  
             }           
...
public function monthlyrepayments(){
  ...
  public function automated(){
      ...
      $hod = DB::table('employee')
            ->join('job_group', 'employee.job_group_id', '=', 'job_group.id')
            ->where('job_group.job_group_name', 'HOD')
      ...
      Leaveapplication::applicationreport($department->id);

      $name = $hod->first_name.' '.$hod->middle_name.' '.$hod->last_name;
      
      $type = "";

      Mail::send( 'emails.automated', array('name'=>$name, 'type'=>"Applied"), function( $message ) use ($hod, $department)
       {
          $message->to($hod->email_office)->subject( 'Leaves Application Report' );
          $message->attach(Leaveapplication::applicationreport($department->id));
       });

      unlink(Leaveapplication::applicationreport($department->id));

      Leaveapplication::approvereport($department->id);

      Mail::send( 'emails.automated', array('name'=>$name, 'type'=>"Approved"), function( $message ) use ($hod, $department)
       {
          $message->to($hod->email_office)->subject( 'Leaves Approved Report' );
          $message->attach(Leaveapplication::approvereport($department->id));
       });

      unlink(Leaveapplication::approvereport($department->id));

      Leaveapplication::rejectreport($department->id);

      Mail::send( 'emails.automated', array('name'=>$name, 'type'=>"rejected"), function( $message ) use ($hod, $department)
       {
          $message->to($hod->email_office)->subject( 'Leaves Rejected Report' );
          $message->attach(Leaveapplication::rejectreport($department->id));
       });

      unlink(Leaveapplication::rejectreport($department->id));

      Leaveapplication::employeereport($department->id);

      Mail::send( 'emails.automated', array('name'=>$name, 'type'=>"employee"), function( $message ) use ($hod, $department)
       {
          $message->to($hod->email_office)->subject( 'Employees on Leave Report' );
          $message->attach(Leaveapplication::employeereport($department->id));
       });

      unlink(Leaveapplication::employeereport($department->id));

      foreach ($leavetypes as $leavetype) {
        Leaveapplication::balancereport($department->id,$leavetype->id);

        Mail::send( 'emails.automated', array('name'=>$name, 'type'=>"balances"), function( $message ) use ($hod, $department, $leavetype)
       {
          $message->to($hod->email_office)->subject( 'Leave Balances Report' );
          $message->attach(Leaveapplication::balancereport($department->id,$leavetype->id));
       });

      unlink(Leaveapplication::balancereport($department->id,$leavetype->id));

      }

     foreach ($employees as $employee) { 
      Leaveapplication::individualreport($department->id,$employee->id);

      Mail::send( 'emails.automated', array('name'=>$name, 'type'=>"individual"), function( $message ) use ($hod, $department, $employee)
       {
          $message->to($hod->email_office)->subject( $employee->first_name.' '.$employee->middle_name.' '.$employee->last_name.' Leave Application Report' );
          $message->attach(Leaveapplication::individualreport($department->id,$employee->id));
       });

      unlink(Leaveapplication::individualreport($department->id,$employee->id));
     }
    }
   }
